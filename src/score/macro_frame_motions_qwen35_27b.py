"""
Macro-Frame Annotation via LLM (Gold Standard Only)
============================================================
Scores only the gold standard motions (EXP_15) on 7 macro-frames
using Qwen3.5-27B-Instruct, for validation purposes.

Input:  results/motions/gold_macro.csv
Output: results/motions/macro_scores/qwen3.5-27b-it.csv
"""
...
MOTIONS_PATH   = "results/motions/climate_motions.csv"
TEXT_COL       = "normalized_text"
BATCH_SIZE     = 16
MAX_NEW_TOKENS = 512  # thinking model needs more tokens
...
FRAMES = {
    "economic": {
        ...
        "anchors": {
            ...
            2: "An economic term appears once as context or background. It does not justify the request.",
        ...
        "examples": {
            ...
            2: "The motion notes that the energy transition will involve transition costs, then focuses on a renewable energy roadmap. -- Cost mentioned once, not used as argument.",
            3: "The motion argues the subsidy scheme will create 5,000 jobs and stimulate clean-tech innovation. -- Economic benefit argued but core request is regulatory.",
            4: "The motion argues ETS reform will cost Dutch industry EUR 2 billion and threaten 12,000 jobs. -- Economic framing dominates alongside one other frame.",
            5: "The motion argues climate levies destroy purchasing power, make industry uncompetitive, and must be replaced by market instruments. -- Pure economic reasoning.",
        },
    ...
    "moral": {
        ...
        "examples": {
            ...
            5: "The text argues we owe it to our children, to developing nations, and to future generations. Acting is a moral imperative. -- Pure moral logic.",
        },
    ...
    "scientific": {
        ...
        "examples": {
            ...
            4: "The text cites IPCC thresholds, PBL policy gap analysis, and KNMI projections. -- Evidence-based reasoning is primary.",
            5: "Every clause cites a specific IPCC AR6 conclusion or PBL projection, and the request follows directly. -- Pure evidence-based reasoning.",
        },
    ...
    "security": {
        ...
        "examples": {
            ...
            4: "The text frames wind and solar expansion as a NATO energy security matter. -- Security framing central.",
            5: "Every clause connects the energy transition to national defence. -- Pure security logic.",
        },
    ...
    "crisis_urgency": {
        ...
        "examples": {
            ...
            4: "The text invokes climate tipping points, ecosystem collapse, and the closing 1.5C window. -- Crisis framing dominant.",
...
def parse_score(response: str) -> tuple[int, bool]:
    """Extract a 1-5 score, stripping thinking tags if present."""
...
def score_frame(
    ...
    """Score all motions on a single frame with checkpointing."""
    ...
    for batch_start in tqdm(range(0, len(todo), BATCH_SIZE), desc=f"  {label}", unit="batch"):
        ...
        for pos, (row_i, response) in enumerate(zip(batch_idx, responses)):
            ...
            if batch_start == 0 and pos < 3:
                print(f"\n  [SAMPLE row={row_i}] '{response.strip()[:100]}' -> {score}")
...
def main():
    # Load gold standard IDs
    gold = pd.read_csv(GOLD_PATH)
    gold_ids = set(gold["id"].astype(str))
    print(f"Gold standard: {len(gold_ids)} motions")

    # Load full motions and filter to gold set only
    motions = pd.read_csv(MOTIONS_PATH)
    motions["id"] = motions["id"].astype(str)
    df = motions[motions["id"].isin(gold_ids)].copy()
    df = df[df[TEXT_COL].notna() & (df[TEXT_COL].str.strip().str.len() > 0)].copy()
    df = df.reset_index(drop=True)
    N = len(df)
    print(f"Motions matched to gold: {N}")

    log_event({"event": "data_loaded", "n_rows": N, "model": MODEL_NAME})
    ...
    log_event({"event": "run_start", "n_rows": N, "frames": FRAME_KEYS})

    # Score all frames
    for frame_key in FRAME_KEYS:
        df[frame_key] = score_frame(frame_key, df, model, tokenizer)

    # Derived metadata
    df["dominant_frame"]       = df[FRAME_KEYS].idxmax(axis=1)
    df["dominant_frame_score"] = df[FRAME_KEYS].max(axis=1)
    df["n_frames_present"]     = df[FRAME_KEYS].apply(lambda r: (r >= 3).sum(), axis=1)
    df["has_parse_failure"]    = df[FRAME_KEYS].apply(lambda r: (r == -1).any(), axis=1)

    # Save
    scores_path = OUT_DIR / f"scores_{MODEL_SHORT}.csv"
    df.to_csv(scores_path, index=False)
    print(f"\nSaved: {scores_path} ({N} rows)")

    # Report
    print(f"\nFINAL REPORT — {MODEL_SHORT} (N={N})")
    for k in FRAME_KEYS:
        valid = df[k][df[k] != -1]
        dist  = " | ".join(f"{i}:{(valid == i).sum()}" for i in range(1, 6))
        print(f"  {FRAMES[k]['label']:<30}  mean={valid.mean():.2f}  [{dist}]")

    log_event({"event": "run_complete", "output": str(scores_path)})
